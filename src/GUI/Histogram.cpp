...
double xStep = xbuck/(double)xbuck2;  
    for(int t = 0; t < tbuck2; t ++) {
      for(int y = 0; y < ybuck2; y ++) {
        for(int x = 0; x < xbuck2; x ++ ) {  //starting x from 0 is generating 19 values for xbuck instead of 18. it was becouse of 
					     //precision problem of double and using it in for loop
		double tt=t*tStep;
		double xx=x*xStep;
		double yy=y*yStep;
          vals.push_back(bFinalValue(xx, yy, tt));
          i++;
...
double Histogram::bFinalValue(double xb, double yb, double tb) { //note that here tt and ty are 0 hence no interpolation is required, but still
    ...
    int
    tbl = (int)tb%tbuck, tbr = (int)ceil(tb)%tbuck,
    xbl = (int)xb%xbuck, xbr = (int)ceil(xb)%xbuck,
    ybl = (int)yb%ybuck, ybr = (int)ceil(yb)%ybuck;
    double tt = tb-tbl, tx = xb-xbl, ty = yb-ybl; 
