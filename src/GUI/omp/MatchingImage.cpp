...
void MatchingImage::castVotes(Mat& img){
this->img=img;
votes.clear(); //applied to remove all previous values when new img is calculated
descriptor.setImage(img);
	grid = Grid(img.cols,img.rows);
	vector<bool> desc;
	#pragma omp parallel for
	for(int i=0;i<grid.cells.size();i++){
		//cout << i << " ---------------------------- " << omp_get_num_threads() << endl; 
...
void MatchingImage::fineAlignment(Mat matchingImg,Mat testImg, int& final_dx,int& final_dy){
	...
	Mat o_Mimg = Mat::zeros(img.rows,img.cols,CV_64F);
	Mat m_Mimg = Mat::zeros(img.rows,img.cols,CV_64F);

	Mat o_Timg = Mat::zeros(img.rows,img.cols,CV_64F);
	Mat m_Timg = Mat::zeros(img.rows,img.cols,CV_64F);
		...
		for(int i=0;i<matchingImg.rows;i++){
			for(int j=0;j<matchingImg.cols;j++){
				if(valid(i+dx+x,matchingImg.rows) && valid(j+dy,matchingImg.cols))
					tmp+=abs(sin(o_Timg.at<double>(i,j)))*testImg.at<uchar>(i,j)*abs(sin(o_Mimg.at<double>(i+dx+x,j+dy)))*matchingImg.at<uchar>(i+dx+x,j+dy);  //check if doing abs is correct
			}
		}
		...
		for(int i=0;i<matchingImg.rows;i++)
			for(int j=0;j<matchingImg.cols;j++)
				if(valid(i+dx,matchingImg.rows) && valid(j+dy+y,matchingImg.cols))
					tmp+=abs(cos(o_Timg.at<double>(i,j)))*testImg.at<uchar>(i,j)*abs(cos(o_Mimg.at<double>(i+dx,j+dy+y)))*matchingImg.at<uchar>(i+dx,j+dy+y);
		if(tmp>maximum){
			final_dy = dy + y;
			maximum = tmp;
		}
...
Weight=(WeightImage(maxNumber,img));
	...
	for(unordered_map<int,ImageVotes>:: iterator it=votes.begin();it!=votes.end();it++){
		...
		mImagestemp.push_back((it->second.maxbinX-2)*grid.cellOffset);
		mImagestemp.push_back((it->second.maxbinY-2)*grid.cellOffset);
